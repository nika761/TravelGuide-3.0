...
public class StoryRecycler extends RecyclerView{
    ...
    private StoryRecycler.VolumeState volumeState;

    public StoryRecycler(@NonNull Context context) {
        super(context);
        init(context);
    }

    public StoryRecycler(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
    ...
    private void init(Context context) {
        ...
        setVolumeControl(StoryRecycler.VolumeState.ON);
...
//        constraintLayout = holder.itemView.findViewById(R.id.layout_post);
    ...
    private void addVideoView() {
        constraintLayout.addView(videoSurfaceView);
...
//            thumbnail.setVisibility(VISIBLE);
    ...
    private void toggleVolume() {
        if (videoPlayer != null) {
            if (volumeState == StoryRecycler.VolumeState.OFF) {
                Log.d(TAG, "togglePlaybackState: enabling volume.");
                setVolumeControl(StoryRecycler.VolumeState.ON);

            } else if (volumeState == StoryRecycler.VolumeState.ON) {
                Log.d(TAG, "togglePlaybackState: disabling volume.");
                setVolumeControl(StoryRecycler.VolumeState.OFF);

            }
        }
    }

    private void setVolumeControl(StoryRecycler.VolumeState state) {
        volumeState = state;
        if (state == StoryRecycler.VolumeState.OFF) {
            videoPlayer.setVolume(0f);
            animateVolumeControl();
        } else if (state == StoryRecycler.VolumeState.ON) {
    ...
    private void animateVolumeControl() {
        if (volumeControl != null) {
            ...
            if (volumeState == StoryRecycler.VolumeState.OFF) {
                requestManager.load(R.drawable.icon_volume_down)
                        .into(volumeControl);
            } else if (volumeState == StoryRecycler.VolumeState.ON) {
                requestManager.load(R.drawable.icon_volume_up)
                        .into(volumeControl);
            }
