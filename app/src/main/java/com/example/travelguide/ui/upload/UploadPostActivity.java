...
import static com.example.travelguide.helper.HelperSystem.WRITE_EXTERNAL_STORAGE;
...
import static com.example.travelguide.ui.music.ChooseMusicActivity.MUSIC_ID;
...
//                s3Helper.upload(files, this);
    ...
    public void onPostUploadedToS3() {
        ...
        if (url.endsWith(".mp4")) {
            ...
            Log.e("zxcv", videoDuration);
        } else {
            stories.add(new UploadPostRequestModel.Post_stories(url, "10", 1, 0));
        }

        UploadPostRequestModel uploadPostRequestModel = new UploadPostRequestModel(stories,
                users, hashtags, musicId, latLng, address, addressName, description, "sometitle");
        uploadPostPresenter.uploadStory(ACCESS_TOKEN_BEARER + HelperPref.getAccessToken(this),
                uploadPostRequestModel);

        Log.e("zxcv", "start upload to iver ");
